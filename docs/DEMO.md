...
**ECC Solar internal tool · Proprietary (see LICENSE) · Build: Piece 5.3**

Job Creator manages the client → job → compliance pipeline for ECC Solar's
...
3. **Job form** — ECC's real fields: site location (kmz-ready) with
...
5. **Rules manager** — every "if this field, then this requirement" is a
   ...
   ECC's requirement matrices and the June 2026 reference documents.
