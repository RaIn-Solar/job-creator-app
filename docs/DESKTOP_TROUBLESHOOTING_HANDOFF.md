# Handoff — Solbiz Desktop won't launch on some devices

**For:** a fresh Claude thread helping Rachel (ECC Solar, non-technical, Windows,
GitHub Desktop) get the Solbiz demo running on her teammates' machines.
...
- Entry point: **`desktop/run_solbiz.py`** — sets the data folder to
  `C:\Users\<name>\Solbiz`, imports the Flask `app`, runs `init_db()`, picks a
  free localhost port, starts the dev server, and opens the browser. On success
  the **console stays open** (the server blocks) and the browser opens Solbiz.
- Recipe: **`desktop/solbiz.spec`** — PyInstaller **one-folder** build
  (`COLLECT`, `console=True`). Output is `dist/Solbiz/` containing
  **`Solbiz.exe` PLUS an `_internal/` folder** (bundled Python, Flask, the
  Jinja `templates/`, and `schema.sql`). **The exe cannot run without its
  `_internal/` folder sitting next to it.**
- Build script: **`desktop/Build-Solbiz-Windows.bat`** (run on Windows).
- Data (the SQLite DB + uploads) lives OUTSIDE the app, in
  `C:\Users\<name>\Solbiz`, so updates don't wipe data.
...
As of **Piece 11.1**, `desktop/run_solbiz.py` was hardened so that any startup
crash is (a) written to **`C:\Users\<name>\Solbiz\solbiz-startup-error.log`**
and (b) shown in a window that **waits for a keypress** instead of closing.
Steps: in GitHub Desktop **Pull origin**, re-run
`desktop/Build-Solbiz-Windows.bat`, reship the new `Solbiz` folder, and have
...
### 1. Only `Solbiz.exe` was copied, not the whole `Solbiz` folder  ← most common
A one-folder PyInstaller build needs `Solbiz.exe` **and** its `_internal/`
...
- **Check:** on the failing device, is there an **`_internal`** folder in the
  same directory as `Solbiz.exe`? If not, this is it.
- **Fix:** transfer the **entire `Solbiz` folder** (zip the whole folder, send
...
- **Check:** does the AV history/quarantine show `Solbiz.exe` or files under
  `_internal`? Does the folder still contain all its files after the flash?
- **Fix:** restore from quarantine and whitelist the `Solbiz` folder (it's
  first-party software). Longer term, code-signing avoids this.

### 4. Can't create the data folder / permissions
`run_solbiz.py` writes to `C:\Users\<name>\Solbiz`. On a locked-down/roaming
profile this can fail.
- **Check:** error mentions `PermissionError` or a path under `\Users\...\Solbiz`.
- **Fix:** ensure the user profile is writable; as a test, the data dir can be
  redirected by setting the `SOLBIZ_DATA_DIR` environment variable to a
...
- [ ] The **full error text** from Option A or the contents of
      `solbiz-startup-error.log` from Option B.
- [ ] Is there an **`_internal`** folder next to `Solbiz.exe` on the failing
      device? (yes/no)
...
- Relevant files to look at: `desktop/run_solbiz.py`, `desktop/solbiz.spec`,
  `desktop/Build-Solbiz-Windows.bat`, `desktop/README-DESKTOP.md`, and `app.py`
